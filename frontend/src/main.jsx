...
import React, { useEffect, useState } from "react";
import { createRoot } from "react-dom/client";
import { Upload, Send, Plus, Trash2, FileText, BarChart3, LogOut } from "lucide-react";
...
function App() {
  ...
  async function handleUpload() {
    ...
    try {
      const results = await uploadMultipleDocuments(files);
      const totalChunks = results.reduce((sum, item) => sum + item.chunks_added, 0);

      setStatus(`${results.length} document(s) indexed successfully. Total chunks: ${totalChunks}`);
      setFiles([]);
      await refreshDocuments();
      await refreshSummary();
    } catch (e) {
  ...
  async function handleAsk(e) {
    ...
    try {
      ...
      setMessages((m) => [
        ...m,
        { role: "assistant", text: `Error: ${e.message}`, sources: [] },
      ]);
    } finally {
      setLoading(false);
    }
    ...
    return (
      <main className="auth-page">
        <section className="auth-card">
          <h1>Production RAG Assistant</h1>
          <p>Login to access your private documents and chats.</p>
          ...
          {authStatus && <p className="status error">{authStatus}</p>}
        </section>
      </main>
    );
  }
  ...
  return (
    <main className="layout">
      ...
      <section className="app">
        ...
        <section className="dashboard">
          <div className="metric"><BarChart3 size={18} /><span>Total Queries</span><b>{summary?.total_queries ?? 0}</b></div>
          <div className="metric"><FileText size={18} /><span>Documents</span><b>{summary?.total_documents ?? 0}</b></div>
          <div className="metric"><span>Chunks</span><b>{summary?.total_chunks ?? 0}</b></div>
          <div className="metric"><span>No Answer</span><b>{summary?.no_answer_count ?? 0}</b></div>
          <div className="metric"><span>Avg Latency</span><b>{summary?.average_latency_ms ?? 0} ms</b></div>
        </section>
        ...
        <section className="card upload-card">
          ...
          {files.length > 0 && <p className="status">Selected: {files.map((file) => file.name).join(", ")}</p>}
          {status && <p className="status">{status}</p>}
        </section>
        ...
        <section className="card documents-card">
          ...
          {documents.map((doc) => (
            <div key={doc.document_id} className="document-row">
              <div>
                <b>{doc.filename}</b>
                <p>Chunks: {doc.chunks_added} | ID: {doc.document_id.slice(0, 8)}</p>
              </div>

              <button className="delete" type="button" onClick={() => handleDeleteDocument(doc.document_id)}>
                <Trash2 size={16} /> Delete
              </button>
            </div>
          ))}
